...
async def start_command(client: Midukki_RoboT, message: message()):
    ...
    if len(message.command) != 2:
        if Configs.START_PICS:
            try:
                await message.reply_photo(photo=choice(Configs.START_PICS), caption=Configs.START_MESSAGE.format(bot=bot_mention, mention=mention, name=bot_name, username=bot_username), replay_markup=markup()(vars.start_buttons))         
            except Exception as e:
                await message.reply_photo(photo=choice(Configs.START_PICS), caption=START_TXT.format(bot=bot_mention, mention=mention, name=bot_name, username=bot_username), replay_markup=markup()(vars.start_buttons))         
...
async def help_command(client: Midukki_RoboT, message: message()):
    ...
    bot_username = Bots.BOT_USERNAME    
    if Configs.START_PICS:
        await message.reply_photo(photo=choice(Configs.START_PICS), caption=HELP_TXT.format(bot=bot_mention, mention=mention, name=bot_name, username=bot_username), replay_markup=markup()(vars.help_buttons))         
    else:
...
async def about_command(client: Midukki_RoboT, message: message()):
    ...
    bot_username = Bots.BOT_USERNAME    
    if Configs.START_PICS:
        await message.reply_photo(photo=choice(Configs.START_PICS), caption=ABOUT_TXT.format(mention=mention, name=bot_name, username=bot_username), replay_markup=markup()(vars.help_buttons))         
    else:
